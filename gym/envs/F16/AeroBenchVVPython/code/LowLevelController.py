...
class NNLowLevelController(LowLevelController):
    '''low level flight controller

    gain matrices and equilibrium points are computed by running BuildLqrControllers.py
    '''

    def __init__(self, ctrlLimits, nn_model):
    ...
    def get_u_deg(self, u_ref, f16_state):
        ...
        # Calculate control using neural network
        u_deg[1:4] = self.nn_model.predict(x_ctrl) # Full Control = np.dot(-self.K_lqr, x_ctrl)
        ...
        # Limit aileron from -21.5 to 21.5 deg
        # Limit rudder from -30 to 30 deg
        u_deg_max = np.array([ctrlLimits.ThrottleMax, ctrlLimits.ElevatorMaxDeg, ctrlLimits.AileronMaxDeg, ctrlLimits.RudderMaxDeg])
        u_deg_min = np.array([ctrlLimits.ThrottleMin, ctrlLimits.ElevatorMinDeg, ctrlLimits.AileronMinDeg, ctrlLimits.RudderMinDeg])
        u_deg = np.clip(u_deg, u_deg_min, u_deg_max)
